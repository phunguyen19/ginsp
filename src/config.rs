...
impl Config {
    pub fn read_toml_file(path: &str) -> anyhow::Result<Config, ErrorKind> {
        let toml = std::fs::read_to_string(path).map_err(|err| ErrorKind::ConfigError(ConfigErrorKind::IOError(err)))?;
        let config: Config = toml::from_str(toml.as_str()).map_err(|err| ErrorKind::ConfigError(ConfigErrorKind::TOMLError(err)))?;
        Ok(config)
    }
}
...
mod tests {
    ...
    fn test_read_toml_file_not_exist() {
        ...
        assert!(config.unwrap_err().to_string().starts_with("Config error: IO error: No such file or directory"));
    }
    ...
    fn test_wrong_toml_format() {
        ...
        assert_eq!(config.is_err(), true);
        assert!(config.unwrap_err().to_string().starts_with("Config error: TOML error: "));
    }
